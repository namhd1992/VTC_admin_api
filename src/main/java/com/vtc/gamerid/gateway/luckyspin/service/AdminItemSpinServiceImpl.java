 ...
 */
@Service
public class AdminItemSpinServiceImpl implements AdminItemSpinService {
    private Logger logger = LoggerFactory.getLogger(AdminItemSpinServiceImpl.class);
    @Autowired
    ...
    public ServiceResponse createItemSpin(SpinItemBeanRequest spinItemBeanRequest,
        ...
        try{
            ...
            TblLuckySpinItem itemOfSpin = new TblLuckySpinItem(spinItemBeanRequest.getName(),
                    ...
                    quantity, createBy, spinItemBeanRequest.getStatus(), spinItemBeanRequest.getIsDefault(),
...
//            }
    ...
    @Override
    @Transactional(propagation = Propagation.REQUIRES_NEW)
    public ServiceResponse updateItemSpin(SpinItemBeanRequest spinItemBeanRequest, UserGameRID createBy) {
        try{
...
//                itemOfSpin.setKeyName(VNCharacterUtils.removeAccent(spinItemBeanRequest.getName()));
    ...
    public ServiceResponse deleteItemSpin(Long spintItemId, UserGameRID createBy) {
        try{
            Long userId = createBy.getId();
            if(createBy.getGroupRole().getName().equals("admin")){
                userId = (long) -1;
